...
public class AsyncProcessor {
    ...
    private ConcurrentHashMap<String, QueryTaskHolder> tasks = new ConcurrentHashMap<>();
    ...
    public void put(String id, SqlQueryTask task) {
        if (tasks.putIfAbsent(id, new QueryTaskHolder(id, tasksExecutor, task)) != null) {
            throw new RuntimeException("Query with id " + id + " exist");
        }
    }

    public DataResponse fetch(String id, long timeout) throws TimeoutException {
        QueryTaskHolder task = tasks.get(id);
        if (task == null) {
            throw new RuntimeException("Query " + id + " not found");
        }
    ...
    public void cancel(String id) {
        QueryTaskHolder task = tasks.remove(id);
        if (task != null) {
            task.close(true);
        }
    }
